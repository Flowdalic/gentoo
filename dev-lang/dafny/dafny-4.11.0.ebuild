...
NUGETS="
boogie.abstractinterpretation@3.5.5
boogie.basetypes@3.5.5
boogie.codecontractsextender@3.5.5
boogie.concurrency@3.5.5
boogie.core@3.5.5
boogie.executionengine@3.5.5
boogie.graph@3.5.5
boogie.houdini@3.5.5
boogie.model@3.5.5
boogie.provers.leanauto@3.5.5
boogie.provers.smtlib@3.5.5
boogie.vcexpr@3.5.5
boogie.vcgeneration@3.5.5
castle.core@4.4.0
...
microsoft.build.tasks.core@17.14.7
microsoft.build.utilities.core@17.14.7
...
if [[ "${PV}" == *9999* ]] ; then
	...
	EGIT_REPO_URI="https://github.com/dafny-lang/${PN}"
else
...
PATCHES=(
	...
	"${FILESDIR}/dafny-4.11.0-lit-config.patch"
)
...
src_prepare() {
	# Ensure we do not accidentally pull in any .NET tools
	# (tool restore is called) in custom Dafny MSBuild steps.
	rm -f dotnet-tools.json \
		|| die "Failed to remove Dotnet Tools JSOB manifest file"

	# Fragile for patching, we have to use sed here.
	sed -e "s|dotnet tool run ||g" \
		-i ./Source/DafnyCore/DafnyCore.csproj \
		|| die
